...
module.exports = PlaceFormView.extend({
  setLocation: function(locationData) {
    ...
    if ($approxAddress.length === 0) {
      $approxAddress = $('<p class="approx-address"></p>').insertAfter(
        $placeNameField,
      );
    }
    ...
    $.ajax({
      url:
        "https://shareabouts-region-service.herokuapp.com/api/v1/greensboro/districts-2015?ll=" +
      ...
      success: function(data) {
        ...
        if (_(closedDistricts).contains(data["CounDist"])) {
          $district.prepend(
            '<span class="closed-district">Please note that the input period for this district is over for 2015.</span> ',
          );
