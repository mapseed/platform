...
var PlaceListItemView = Backbone.Marionette.Layout.extend({
  ...
  initialize: function() {
    ...
    this.model.submissionSets[supportType] =
      this.model.submissionSets[supportType] ||
    ...
    this.supportView = new SupportView({
      collection: this.model.submissionSets[
        Shareabouts.Config.support.submission_type
      ],
  ...
  applyFilters: function(filters, term, max) {
    ...
    this.unrenderedItems.each(function(model, index) {
      ...
      if (submitter) {
        if (
          (submitter.name &&
            submitter.name.toUpperCase().indexOf(term) !== -1) ||
          (submitter.username &&
            submitter.username.toUpperCase().indexOf(term) !== -1)
        ) {
          previouslyUnrenderedModels.push(model);
          return this.collection.add(model);
        }
