...
module.exports = Backbone.View.extend({
  initialize: function() {
    ...
    this.$el.delegate("a", "click", function(evt) {
      ...
      self.options.router.navigate(this.getAttribute("href"), {
        trigger: true,
      });
    });
  ...
  checkForNewActivity: function() {
    ...
    options.complete = _.bind(function() {
      ...
      this.newContentTimeout = setTimeout(
        _.bind(this.checkForNewActivity, this),
        this.interval,
      );
    }, this);
  ...
  processActionData: function(actionModel, placeModel) {
    ...
    if (placeType) {
      ...
      actionData.target.submitter_name =
        actionModel.get("target").submitter_name || anonSubmitterName;
      ...
      if (modelData) {
        ...
        if (index >= self.$el.children().length) {
          ...
          $template
            ...
            .insertBefore(
              self.$el.find(".activity-item:nth-child(" + index + 1 + ")"),
            )
  ...
  render: function() {
    ...
    $template = Handlebars.templates["activity-list"]({
      activities: collectionData,
    });
